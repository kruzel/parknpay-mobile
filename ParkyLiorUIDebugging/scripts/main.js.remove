(function($, doc) {
	var _app,
    	...
    	_storeListElem,
    	_private,
   	 _parkyAppData = new parkyAppData(),
    	_isOnline = true,
    	_tmplStoreList;
	...
	_private = {
		...
		initMap: function(position) {
			...
			// Get stores nearby
			_parkyAppData.getParkingsLocations(position.coords.latitude, position.coords.longitude)
	...
	_app = {
		init: function() {
			announcementViewModel.load(_parkyAppData.getAnnouncements());
            
            if (window.localStorage.getItem("cars") === null) {
				localStorage.setItem("cars", _parkyAppData.getInitialCars());
			}
            
            carsViewModel.loadFromLocalStorage();
		},
        
		onAddCarViewShow: function () {
            addCarViewModel.resetView();
		},
		...
		rewardCardInit: function(e) {
			...
			singleCarViewModel.appendCarFadeEffect($cardFront, $cardBack);
		},
        
		singleCarShow: function (args) {
			var cardNumber = args.view.params.cardNumber,
    			bonusPoints = args.view.params.bonusPoints,
    			cardAmount = args.view.params.cardAmount;
            
			singleCarViewModel.setValues(cardNumber, bonusPoints, cardAmount);
		},
        
		singleCarInit: function(e) {
			var container = e.view.content,
    			$cardFront = container.find("#cardFront"),
    			$cardBack = container.find("#cardBack");
            
			singleCarViewModel.appendCardFadeEffect($cardFront, $cardBack);
		},
	...
	$.extend(window, {
		cardsViewModel: _app.carsViewModel,
		rewardCardShow: _app.rewardCardShow,
		rewardCardInit: _app.rewardCardInit,
		singleCardShow: _app.singleCarShow,
		singleCardInit: _app.singleCarInit,
		onAddCardViewShow: _app.onAddCarViewShow,
